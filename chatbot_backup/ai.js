...
router.post('/ask', async (req, res) => {
    try {
        ...
        const { buildRepoContext } = await import('../services/githubService.js');
        const owner = process.env.GITHUB_OWNER;
        const repoContext = await buildRepoContext(owner, repo);

        // Call AI service with structured context
        const response = await aiService.askAnything(question, repoContext);
